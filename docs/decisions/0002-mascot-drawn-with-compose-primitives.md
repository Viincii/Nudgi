...
Draw Nudgi in code with `Canvas`, `Path` and Compose animation APIs (`core:mascot`). A mood maps to continuous facial
parameters (`MascotFace`) that are animated with springs, plus an infinite idle transition for breathing and blinking.

## Consequences

- No asset pipeline and no extra dependency; the whole mascot is a few hundred lines.
